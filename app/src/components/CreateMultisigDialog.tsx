...
interface CreateMultisigDialogProps {
  onCreateMultisig: (multisig: Omit<Multisig, 'id' | 'createdAt' | 'txPending' | 'transactionCount' | 'creator' | 'vaultaddress'>) => void;
  children: ReactNode;
}
...
export function CreateMultisigDialog({ onCreateMultisig, children }: CreateMultisigDialogProps) {
  ...
  const [owners, setOwners] = useState<string[]>([ wallet.publicKey?.toBase58() ?? '', '']);
